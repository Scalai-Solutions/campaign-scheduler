...
const worker = new Worker('batch.reconcile', async (job) => {
    ...
    try {
        ...
        try {
            ...
            // Reschedule reconciliation in 30 seconds
        ...
        // Group Retell results by metadata.nextStepIntentId
        ...
        // 6. Handle based on semantic state
        ...
        const job = await queues.batchReconcile.getJob(job.id);
        const nextDelay = Math.min((job?.attemptsMade || 0) * 30000, 300000); // Cap at 5 min
        await queues.batchReconcile.add(
            `reconcile-${batchDispatchId}`,
            { batchDispatchId },
            {
...
async function handleFullSuccess(batch, intents, callResultsByIntentId, session) {
    ...
    batch.successCount = intents.length;
...
async function handlePartialFailed(batch, states, callResultsByIntentId, session) {
    ...
    for (const failedIntent of failed) {
        ...
        if (retryCount < maxRetries) {
            ...
            batch.failureCount = (batch.failureCount || 0) + 1;
        } else {
    ...
    batch.successCount = completed.length;
...
async function handleTimeout(batch) {
    ...
    try {
        ...
        batch.failureCount = batch.leadCount;
